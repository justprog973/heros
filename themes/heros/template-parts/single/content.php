<article <?php post_class("jp-article bg-white border-t border-gray-100") ?>>
    <?php if (get_the_post_thumbnail() !== ""): ?>
        <header style="background-image: url(<?php the_post_thumbnail_url('full') ?>)" class="jp-article__header before:bg-black/75 before:h-full before:w-full before:absolute relative card-thumbnail w-full h-[200px] md:h-[250px] bg-no-repeat bg-center bg-cover">
    <?php else: ?>
   ...
   <?php endif; ?>
            <div class="header-single-container relative z-10 jp-container h-full flex justify-start items-center">
                <div class="flex flex-col">
                    ...
                    <p class="text-slate-300 text-sm md:text-dase italic mb-2"><?php _themename_post_meta(); ?></p>
                    <h1 class="card-title">
                        <a class="card-title__link text-2xl md:text-5xl text-white" href="<?php the_permalink(); ?>" title="<?php the_title_attribute() ?>">
                            <?php the_title(); ?>
                        </a>
                    </h1>
                </div>
